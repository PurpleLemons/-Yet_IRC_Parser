...
server.route({
    ...
    handler: function (request, reply) {
    	...
    	do
    	{
    		...
    		var fs = require('fs');
    		if(fs.existsSync(oldPath + '&yet-' + strYear + '-' + strMonth + '-' + strDay + '.log'))
...
server.route({
	...
	handler: function (request, reply) {
		...
		{
			if(fsCheck.existsSync(oldPath+"&yet-"+strYear+"-"+strMonth+"-0"+i+".log"))
		...
		while(startDate.getDate() <= maxDays && +strMonth == startDate.getMonth()+1)
		{
			...
			if(fsCheck.existsSync(oldPath+"&yet-"+strYear+"-"+strMonth+"-"+strDay+".log"))
		...
		while(prevDone==false)
		{
			...
			if(fsCheck.existsSync(oldPath+"&yet-"+prevYear+"-"+prevMonth+"-"+prevDay+".log"))
			{
		...
		while(nextDone==false)
		{
			...
			if(fsCheck.existsSync(oldPath+"&yet-"+nextYear+"-"+nextMonth+"-"+nextDay+".log"))
			{
		...
		var fileStr=fsCheck.readFileSync(oldPath+"&yet-"+strYear+"-"+strMonth+"-"+request.params.file.substring(3,5)+".log").toString().split('\n');//Makes it into a string array
